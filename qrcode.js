...
(function () {
	...
	QRCode = function (text, correctLevel) {
		...
		correctLevel = correctLevel || QRErrorCorrectLevel.H;

		let code = new QRCodeModel(_getTypeNumber(text, correctLevel), correctLevel);
		code.addData(text);
		code.make();
		...
		for(let y = 0; y<code.modules.length; ++y){
			...
			for(let x = 0; x<module.length; ++x){
				if(module[x])
					out += '<rect x='+x+' y='+y+' width=1 height=1 style=\"fill:#000;\" />';
			}
		}
		out+= '</svg>';
		return out;
